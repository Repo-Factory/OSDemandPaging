...
void replacePageIfNecessary(PageTable& pageTable, Ring& circularList, const unsigned int virtualAddress, const int threshold)
{
    if (circularList.current_index == circularList.capacity) {
        pageReplaceClock(circularList, findAvailablePage(circularList, threshold), virtualAddress);
...
int main(int argc, char* argv[])
{
    ...
    [&](const std::string virtualAddressFromFile, const std::string accessModeFromeFile) {
        ...
        replacePageIfNecessary(pageTable, circularList, std::stoi(virtualAddressFromFile), args.optionalArgs.a_flag);
        switch(args.optionalArgs.l_flag)
        {
            ...
            case LoggingMode::vpns_pfn:
                log_vpns_pfn(args.mandatoryArgs.levelBits, , sequentialFrameNumber);
                break;
